<?php
/**
 * @package plg_jshoppingadmin_watermark
...
class plgJshoppingAdminWatermark extends JPlugin
{
    ...
    protected function addWatermark($nameFull, $nameImage, $nameThumb)
    {
        ...
        if (empty($pattern)) {
            $app->enqueueMessage(sprintf(JText::_('PLG_JSHOPPINGADMIN_WATERMARK_PATTERN_PATH_EMPTY'), JText::_('PLG_JSHOPPINGADMIN_WATERMARK')), 'warning');
            return false;
        }

        $patternPath = JPATH_SITE . '/' . $pattern;
        if (!file_exists($patternPath)) {
            $app->enqueueMessage(JText::_('PLG_JSHOPPINGADMIN_WATERMARK_PATTERN_FILE_NOT_FOUND'), 'warning');
            return false;
        }
