...
function QuantityPlus($document, quantityId){
    
    let $input = $('.quantity input[name="' + quantityId +'"]');
    let count = parseInt($input.val()) + 1;
    $input.val(count);
    doSubmit($document); 
};

function QuantityMinus($document, quantityId){
...
/*Функция добавления во все таблицы класс 'table-wrap' */
...
let burgerCat = $('.mob-menu-cat');
burgerCat.click(function() {
  if (!$(this).hasClass('clicked')) {
	$(this).addClass('clicked');
	$('.moduletablenavmenucat').css({
	  'left': '0px'
	});
  } else {
...
$('body').mouseup(function(e) {
    var burger_div = $(".nav");
    if (!burger_div.is(e.target) && burger_div.has(e.target).length === 0 && !burgerItem.is(e.target)) {
      $('.nav').css({
        'left': '-100%'
      });
...
$('body').mouseup(function(e) {
    var burger_div = $(".moduletablenavmenucat");
    if (!burger_div.is(e.target) && burger_div.has(e.target).length === 0 && !burgerItem.is(e.target)) {
      $('.moduletablenavmenucat').css({
        'left': '-100%'
      });
